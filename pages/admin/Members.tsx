...
const Members: React.FC = () => {
  ...
  const handleUnverify = async (memberId: string) => {
    if (!confirm('Are you sure you want to remove verification from this member?')) return
    
    const { error } = await adminMembersService.unverifyMember(memberId)
    if (error) {
      console.error('Error unverifying member:', error)
      alert('Failed to unverify member')
      } else {
      alert('Member verification removed successfully!')
      fetchMembers()
    }
  }
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Manage and verify community members
            </p>
          </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  <div>
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(member.user_status)}`}>
            ...
            {members.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 max-w-md mx-auto">
                  {filter === 'verified' 
                    ? 'No verified members yet. Start verifying members to see them here.'
                    : filter === 'unverified'
